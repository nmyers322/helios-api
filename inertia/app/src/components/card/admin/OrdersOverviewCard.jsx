...
import { getAllOrdersByStatus } from "../../../modules/heliosApi";
...
const OrdersOverviewCard = ({statuses=null}) => {
    ...
    heliosLogger("OrdersOverviewCard", "openOrders", openOrders);
    ...
    useEffect(() => {
        async function fetchOrders() {
            ...
            let allOrders = await getAllOrdersByStatus("CREATED");
    ...
    return (
        <Card title="Orders Overview">
            ...
            { openOrders && Object.keys(openOrders).map((index) => (
                <OrderRow key={index} onClick={() => setSelectedOrder(index)}>
                    <OrderDetail>
                        {openOrders[index].id}
                    </OrderDetail>
                    <OrderDetail>
                        {openOrders[index].status.replace("CREATED", "Unpaid").replace("PAID", "Paid")}
                    </OrderDetail>
                    <OrderDetail>
